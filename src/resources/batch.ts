...
export class BatchResource extends BaseResource {
  ...
  async send(params: BatchSendParams): Promise<BatchSendResult> {
    const body = typeof params.fileContent === 'string'
      ? params.fileContent
      : params.fileContent;
  ...
  async finish(params: BatchFinishParams): Promise<BatchFinishResult> {
    return this.requestJson<BatchFinishResult>(
      'POST',
      `/batch/Finish/${params.referenceNumber}`,
      {
        body: {},
        requestOptions: params.requestOptions,
      },
  ...
  async status(params: BatchStatusParams): Promise<BatchStatusResult> {
    return this.requestJson<BatchStatusResult>(
      'GET',
      `/batch/Status/${params.referenceNumber}`,
      { requestOptions: params.requestOptions },
    );
  }
}
